...
no_face_ind = find(Y_test == -1)
no_face_ind = no_face_ind(1)

face_ind = find(Y_test == 1)
face_ind = face_ind(1)

subplot(1,2,1)
pred = '';
if predictions_test(face_ind) == 1
    pred = 'face';
else
    pred = 'no face';
end
imagesc(reshape(X_test(:,face_ind), 19,19))
title(['Face, predicted as ' pred])

subplot(1,2,2)
pred = '';
if predictions_test(no_face_ind) == 1
    pred = 'face';
else
    pred = 'no face';
end
imagesc(reshape(X_test(:,no_face_ind), 19,19))
title(['No face, predicted as ' pred])